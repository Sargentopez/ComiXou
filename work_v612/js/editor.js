...
// ── Multi-selección ──────────────────────────────────────────────────────────
let edMultiSel = [];   // array de índices de capas seleccionadas
let edMsRubber = null; // {x0,y0,x1,y1} rubber band activa (coords norm)
// Snapshot al inicio de cada gesto (patrón Konva/Excalidraw):
// en lugar de un proxy, guardamos el estado inicial de TODOS los miembros
// y aplicamos el delta directamente a cada objeto en cada frame.
let edMsGesture = null;
// edMsGesture = {
//   type: 'drag'|'resize'|'rotate',
//   startCursor: {nx,ny},         // posición del cursor al empezar
//   startAngle: Number,           // solo para rotate
//   corner: String,               // solo para resize
//   bb: {cx,cy,w,h,x0,y0,x1,y1}, // bbox en el momento de iniciar
//   items: [{idx, x,y,w,h,rot}],  // snapshot de cada miembro
// }
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
/* ════════════════════════════════════════════════════════════
   MULTI-SELECCIÓN  (patrón Konva/Excalidraw)
   ════════════════════════════════════════════════════════════

   Algoritmo: los objetos seleccionados NO se sustituyen por un proxy.
   En su lugar se calcula el AABB colectivo en cada frame para dibujar
   los handles, y al inicio de cada gesto se guarda un snapshot de
   posición/tamaño/rotación de cada miembro. Durante el gesto se aplica
   el delta directamente a cada objeto. Al soltar se descarta el snapshot.
   ════════════════════════════════════════════════════════════ */

// AABB axis-aligned del grupo (coords normalizadas, sin rotación)
function _msBBox(){
  const pw=edPageW(), ph=edPageH();
...
// 8 handles de escala + 1 de rotación en coords normalizadas (para hit-test)
function _msHandlePositions(bb){
  const {cx,cy,w,h}=bb;
  return[
    {id:'tl',x:cx-w/2,y:cy-h/2},{id:'tr',x:cx+w/2,y:cy-h/2},
    {id:'bl',x:cx-w/2,y:cy+h/2},{id:'br',x:cx+w/2,y:cy+h/2},
    {id:'mt',x:cx,    y:cy-h/2},{id:'mb',x:cx,    y:cy+h/2},
    {id:'ml',x:cx-w/2,y:cy    },{id:'mr',x:cx+w/2,y:cy    },
  ];
}

// Dibuja bbox colectivo, contornos individuales, handles escala y handle rotación
function _msDrawGroup(){
  if(!edMultiSel.length) return;
  const bb=_msBBox(); if(!bb) return;
  const pw=edPageW(), ph=edPageH(), z=edCamera.z;
  const lw=1/z, hr=6/z, hrRot=8/z;
  const mx=edMarginX(), my=edMarginY();
  edCtx.save();

  // ── Contornos individuales tenues ──
  for(const i of edMultiSel){
    const la=edLayers[i]; if(!la) continue;
    ...
    edCtx.strokeStyle='rgba(26,140,255,0.4)'; edCtx.lineWidth=lw;
    edCtx.setLineDash([4/z,3/z]); edCtx.strokeRect(-w/2,-h/2,w,h);
    edCtx.setLineDash([]);
    edCtx.restore();
  }
  // ── Bbox colectivo ──
  const bx=mx+bb.x0*pw, by=my+bb.y0*ph, bw=bb.w*pw, bh=bb.h*ph;
  edCtx.strokeStyle='#1a8cff'; edCtx.lineWidth=1.5/z;
  edCtx.setLineDash([6/z,3/z]); edCtx.strokeRect(bx,by,bw,bh); edCtx.setLineDash([]);

  // ── Handles de escala ──
  for(const p of _msHandlePositions(bb)){
    const hx=mx+p.x*pw, hy=my+p.y*ph;
  ...
  // ── Handle de rotación — idéntico al individual en posición y estilo ──
  // El bbox no está rotado → el handle está siempre en el centro-superior del bbox.
  // Posición: 28px físicos de pantalla por encima del borde superior del bbox.
  const rotHx=mx+bb.cx*pw;
  const rotHy=my+bb.y0*ph - 28/z;
  edCtx.beginPath(); edCtx.moveTo(rotHx, my+bb.y0*ph); edCtx.lineTo(rotHx, rotHy+hrRot);
  edCtx.strokeStyle='#1a8cff'; edCtx.lineWidth=lw; edCtx.stroke();
  edCtx.beginPath(); edCtx.arc(rotHx,rotHy,hrRot,0,Math.PI*2);
  edCtx.fillStyle='#1a8cff'; edCtx.fill();
  edCtx.strokeStyle='#fff'; edCtx.lineWidth=lw*1.5; edCtx.stroke();
  const ar=hrRot*0.55;
  edCtx.beginPath(); edCtx.arc(rotHx,rotHy,ar,-Math.PI*0.9,Math.PI*0.5);
  edCtx.strokeStyle='#fff'; edCtx.lineWidth=lw*1.5; edCtx.stroke();
  const ax=rotHx+ar*Math.cos(Math.PI*0.5), ay=rotHy+ar*Math.sin(Math.PI*0.5);
  edCtx.beginPath();
...
// Dibuja solo la rubber band
function _msDrawRubber(){
  if(!edMsRubber) return;
  const pw=edPageW(), ph=edPageH(), z=edCamera.z;
  const x0=Math.min(edMsRubber.x0,edMsRubber.x1), y0=Math.min(edMsRubber.y0,edMsRubber.y1);
  const x1=Math.max(edMsRubber.x0,edMsRubber.x1), y1=Math.max(edMsRubber.y0,edMsRubber.y1);
  edCtx.save();
  edCtx.fillStyle='rgba(26,140,255,0.06)'; edCtx.strokeStyle='#1a8cff';
  edCtx.lineWidth=1.5/z; edCtx.setLineDash([5/z,3/z]);
  const rx=edMarginX()+x0*pw, ry=edMarginY()+y0*ph;
  edCtx.fillRect(rx,ry,(x1-x0)*pw,(y1-y0)*ph);
  edCtx.strokeRect(rx,ry,(x1-x0)*pw,(y1-y0)*ph);
  edCtx.setLineDash([]);
  edCtx.restore();
}

// Limpia toda la multi-selección
function _msClear(){
  edMultiSel=[]; edMsRubber=null; edMsGesture=null;
}
...
function edRedraw(){
  ...
  if(edActiveTool==='multiselect'){
    if(edMultiSel.length) _msDrawGroup();
    else _msDrawRubber();
  }
...
function edOnStart(e){
  ...
  if(edActiveTool==='multiselect'){
    ...
    if(edMultiSel.length){
      const bb=_msBBox();
      if(bb){
        // ¿Hit en handle de rotación del grupo?
        // Posición idéntica al render: 28px físicos encima del borde superior del bbox.
        const rotHx=bb.cx, rotHy=bb.y0-28/(ph*edCamera.z);
        const dRx=(c.nx-rotHx)*pw, dRy=(c.ny-rotHy)*ph;
        if(Math.hypot(dRx,dRy) < 14){
          edMsGesture={
            type:'rotate',
            startAngle: Math.atan2(c.ny-bb.cy, c.nx-bb.cx),
            bb:{...bb},
            items: edMultiSel.map(i=>({idx:i,x:edLayers[i].x,y:edLayers[i].y,rot:edLayers[i].rotation||0})),
          };
          return;
        }
        // ¿Hit en handle de escala?
        for(const p of _msHandlePositions(bb)){
          const dpx=(c.nx-p.x)*pw, dpy=(c.ny-p.y)*ph;
          if(Math.hypot(dpx,dpy)<14){
            edMsGesture={
              type:'resize', corner:p.id,
              startCursor:{nx:c.nx,ny:c.ny},
              bb:{...bb},
              items: edMultiSel.map(i=>({idx:i,x:edLayers[i].x,y:edLayers[i].y,w:edLayers[i].width,h:edLayers[i].height})),
            };
            return;
          }
        }
        // ¿Hit dentro del bbox o en algún miembro? → drag
        const inBbox=c.nx>=bb.x0&&c.nx<=bb.x1&&c.ny>=bb.y0&&c.ny<=bb.y1;
        const inMember=edMultiSel.some(i=>edLayers[i]?.contains(c.nx,c.ny));
        if(inBbox||inMember){
          edMsGesture={
            type:'drag',
            startCursor:{nx:c.nx,ny:c.ny},
            items: edMultiSel.map(i=>({idx:i,x:edLayers[i].x,y:edLayers[i].y})),
          };
          return;
        }
      }
    }
    // Click fuera del grupo (o sin selección) → iniciar rubber band
    _msClear();
    edMsRubber={x0:c.nx,y0:c.ny,x1:c.nx,y1:c.ny};
    edRedraw();
    return;
  }
  // ════════════════════════════════════════════
  ...
  if(_la && _la.type!=='bubble'){
    ...
    // ── Handle de rotación: misma fórmula que el render ──
    // Render dibuja el círculo a (0, -h/2 - 28/z) en espacio local del objeto.
    // h = la.height*ph (px workspace), 28/z son px físicos de pantalla.
    // Convertir a fracción de página: offset = la.height/2 + 28/(ph*z)
    // Luego rotar vector (0, -offset) por la rotación del objeto.
    if(!_isT){
      const _rot = (_la.rotation||0)*Math.PI/180;
      const _offset = _la.height/2 + 28/(_ph * edCamera.z);
      const _rx = _la.x + Math.sin(_rot) * _offset;
      const _ry = _la.y - Math.cos(_rot) * _offset;
      const _ddx = (c.nx - _rx) * _pw, _ddy = (c.ny - _ry) * _ph;
      if(Math.hypot(_ddx, _ddy) < hitPx){
        edIsRotating = true;
        edRotateStartAngle = Math.atan2(c.ny-_la.y, c.nx-_la.x)-(_la.rotation||0)*Math.PI/180;
        return;
      }
    }
    // ── Handles de escala (getControlPoints, saltando 'rotate') ──
...
function edOnMove(e){
  ...
  // ════════════════════════════════════════════
  // MULTI-SELECCIÓN — mover rubber band o aplicar gesto
  // ════════════════════════════════════════════
  if(edActiveTool==='multiselect'){
    // Rubber band
    if(edMsRubber && !edMsGesture){
      const c=edCoords(e); e.preventDefault();
      edMsRubber.x1=c.nx; edMsRubber.y1=c.ny;
      edRedraw(); return;
    }
    // Gesto activo sobre el grupo
    if(edMsGesture){
      const c=edCoords(e); e.preventDefault();
      const pw=edPageW(), ph=edPageH();
      const {type,items,bb}=edMsGesture;

      if(type==='drag'){
        const dx=c.nx-edMsGesture.startCursor.nx;
        const dy=c.ny-edMsGesture.startCursor.ny;
        items.forEach(s=>{ edLayers[s.idx].x=s.x+dx; edLayers[s.idx].y=s.y+dy; });
        window._edMoved=true;

      } else if(type==='rotate'){
        // Rotar todos los objetos alrededor del centro del bbox original
        const curAngle=Math.atan2(c.ny-bb.cy, c.nx-bb.cx);
        const delta=curAngle-edMsGesture.startAngle;
        const cos=Math.cos(delta), sin=Math.sin(delta);
        items.forEach(s=>{
          const la=edLayers[s.idx]; if(!la) return;
          // Rotar la posición alrededor del centro del bbox (en px para no distorsionar)
          const dx_px=(s.x-bb.cx)*pw, dy_px=(s.y-bb.cy)*ph;
          la.x = bb.cx + (dx_px*cos - dy_px*sin)/pw;
          la.y = bb.cy + (dx_px*sin + dy_px*cos)/ph;
          la.rotation = (s.rot + delta*180/Math.PI) % 360;
        });
        window._edMoved=true;

      } else if(type==='resize'){
        // Escalar todos los objetos manteniendo su posición relativa al centro del bbox
        const dx=c.nx-edMsGesture.startCursor.nx;
        const dy=c.ny-edMsGesture.startCursor.ny;
        const {corner}=edMsGesture;
        let sx=1, sy=1;
        if(bb.w>0) {
          if(corner==='tl'||corner==='bl'||corner==='ml') sx=1-dx/bb.w;
          if(corner==='tr'||corner==='br'||corner==='mr') sx=1+dx/bb.w;
        }
        if(bb.h>0) {
          if(corner==='tl'||corner==='tr'||corner==='mt') sy=1-dy/bb.h;
          if(corner==='bl'||corner==='br'||corner==='mb') sy=1+dy/bb.h;
        }
        // Esquinas: mantener proporción
        if(['tl','tr','bl','br'].includes(corner)){
          const s=(Math.abs(sx)+Math.abs(sy))/2; sx=sy=s;
        }
        sx=Math.max(sx,0.05); sy=Math.max(sy,0.05);
        items.forEach(s=>{
          const la=edLayers[s.idx]; if(!la) return;
          la.x = bb.cx + (s.x-bb.cx)*sx;
          la.y = bb.cy + (s.y-bb.cy)*sy;
          la.width  = Math.max(s.w*Math.abs(sx), 0.02);
          la.height = Math.max(s.h*Math.abs(sy), 0.02);
        });
        window._edMoved=true;
      }
      edRedraw(); return;
...
function edOnEnd(e){
  ...
  // ════════════════════════════════════════════
  // MULTI-SELECCIÓN — confirmar rubber band o finalizar gesto
  // ════════════════════════════════════════════
  if(edActiveTool==='multiselect'){
    if(edMsRubber && !edMsGesture){
      // Confirmar rubber band → seleccionar objetos cuyo centro cae dentro
      const rx0=Math.min(edMsRubber.x0,edMsRubber.x1);
      const ry0=Math.min(edMsRubber.y0,edMsRubber.y1);
      const rx1=Math.max(edMsRubber.x0,edMsRubber.x1);
      const ry1=Math.max(edMsRubber.y0,edMsRubber.y1);
      edMsRubber=null;
      if((rx1-rx0)>0.005||(ry1-ry0)>0.005){
        edMultiSel=[];
        edLayers.forEach((la,i)=>{
          if(la.type==='text'||la.type==='bubble'||la.type==='draw') return;
...
function EditorView_init(){
  ...
  // ── Multi-selección: botón + tecla M ──
  (function(){
    function activateMultiSel(){
      _msClear(); edSelectedIdx=-1;
      edActiveTool='multiselect';
      edCanvas.className='tool-multiselect';
      $('edMultiSelBtn')?.classList.add('ms-active');
      const p=$('edOptionsPanel'); if(p){p.classList.remove('open');p.innerHTML='';}
      edRedraw();
    }
    function deactivateMultiSel(){
      _msClear();
      edActiveTool='select';
      edCanvas.className='';
      $('edMultiSelBtn')?.classList.remove('ms-active');
      edSelectedIdx=-1;
      edRedraw();
    }
    $('edMultiSelBtn')?.addEventListener('click',()=>{
      if(edActiveTool==='multiselect') deactivateMultiSel();
      else activateMultiSel();
    });
    if(!window._edMsKeyFn){
      window._edMsKeyFn = ev => {
        const a=document.activeElement;
        if(a&&(a.tagName==='INPUT'||a.tagName==='TEXTAREA'||a.isContentEditable)) return;
        if(ev.key==='m'||ev.key==='M'){
          if(edActiveTool==='multiselect') deactivateMultiSel(); else activateMultiSel();
        }
        if(ev.key==='Escape'&&edActiveTool==='multiselect') deactivateMultiSel();
      };
      document.addEventListener('keydown',window._edMsKeyFn);
    }
  })();
