...
let edActiveTool = 'select';  // select | draw | eraser | multiselect
// ── Multiselección ───────────────────────────────────────────────────────────
let edMultiSel   = [];      // array de índices seleccionados
let edMsRubber   = null;    // {x0,y0,x1,y1} en fracción de página — rubber band activa
let edMsGesture  = null;    // snapshot del gesto activo {type, startX, startY, startAngle, items[]}
// edMsGesture.items[] = [{idx, x, y, width, height, rotation}] — estado inicial de cada miembro
...
// Bounding box axis-aligned del grupo (en fracción de página)
function _msBBox(){
  if(!edMultiSel.length) return null;
  const pw=edPageW(), ph=edPageH();
  let x0=Infinity,y0=Infinity,x1=-Infinity,y1=-Infinity;
  for(const idx of edMultiSel){
    const la=edLayers[idx]; if(!la) continue;
    // Calcular las 4 esquinas del objeto rotado y obtener su AABB
    const rot=(la.rotation||0)*Math.PI/180;
    const hw=la.width/2, hh=la.height/2;
    for(const [dx,dy] of [[-hw,-hh],[hw,-hh],[-hw,hh],[hw,hh]]){
      // Rotar en px para no distorsionar (pw!=ph)
      const rx = (dx*pw*Math.cos(rot) - dy*ph*Math.sin(rot))/pw;
      const ry = (dx*pw*Math.sin(rot) + dy*ph*Math.cos(rot))/ph;
...
// 8 posiciones de handles de escala del bbox colectivo (en fracción de página)
function _msHandlePositions(bb){
  const {x0,y0,x1,y1,cx,cy} = bb;
  return [
    {x:x0,  y:y0,  c:'tl'}, {x:cx,  y:y0,  c:'tc'}, {x:x1,  y:y0,  c:'tr'},
    {x:x1,  y:cy,  c:'mr'}, {x:x1,  y:y1,  c:'br'}, {x:cx,  y:y1,  c:'bc'},
    {x:x0,  y:y1,  c:'bl'}, {x:x0,  y:cy,  c:'ml'},
  ];
}

// Dibujar bbox colectivo + handles (sin rotación del grupo: AABB)
function _msDrawGroup(){
  ...
  edCtx.save();
  // Contornos tenues de cada miembro
  for(const idx of edMultiSel){
    const la=edLayers[idx]; if(!la) continue;
  ...
  // Bbox colectivo
  const bx=mx+bb.x0*pw, by=my+bb.y0*ph;
  const bw=bb.w*pw, bh=bb.h*ph;
  edCtx.strokeStyle='#1a8cff'; edCtx.lineWidth=lw;
  edCtx.setLineDash([5/z,3/z]);
  edCtx.strokeRect(bx,by,bw,bh);
  edCtx.setLineDash([]);

  // Handles de escala
  for(const h of _msHandlePositions(bb)){
    const hx=mx+h.x*pw, hy=my+h.y*ph;
  ...
  // Handle de rotación — encima del borde top-center
  // offset en px-workspace desde el centro del bbox hasta el handle
  const _rotOffWs = bb.h*ph/2 + 28/z;  // px workspace
  const _rotHx = mx + bb.cx*pw;
  const _rotHy = my + bb.cy*ph - bb.h*ph/2 - 28/z;
  edCtx.beginPath(); edCtx.moveTo(_rotHx, my+bb.y0*ph); edCtx.lineTo(_rotHx, _rotHy+hrRot);
  edCtx.strokeStyle='#1a8cff'; edCtx.lineWidth=lw; edCtx.stroke();
  edCtx.beginPath(); edCtx.arc(_rotHx, _rotHy, hrRot, 0, Math.PI*2);
...
function _msDrawRubber(){
  ...
  const mx=edMarginX(), my=edMarginY();
  const r=edMsRubber;
  const x=mx+Math.min(r.x0,r.x1)*pw, y=my+Math.min(r.y0,r.y1)*ph;
  const w=Math.abs(r.x1-r.x0)*pw, h=Math.abs(r.y1-r.y0)*ph;
  edCtx.save();
  edCtx.fillStyle='rgba(26,140,255,0.08)';
  edCtx.fillRect(x,y,w,h);
  edCtx.strokeStyle='#1a8cff'; edCtx.lineWidth=1/z;
  edCtx.setLineDash([4/z,3/z]); edCtx.strokeRect(x,y,w,h); edCtx.setLineDash([]);
  edCtx.restore();
}

// Limpiar toda la multiselección
function _msClear(){
  edMultiSel=[]; edMsRubber=null; edMsGesture=null;
}

function edActivateMultiselTool(){
  _msClear();
  edSelectedIdx=-1;
...
function edDeactivateMultiselTool(){
  _msClear();
  edActiveTool='select';
  edCanvas.className='';
...
function edOnStart(e){
  ...
  if(edActiveTool==='multiselect'){
    const c=edCoords(e);
    const pw=edPageW(), ph=edPageH(), z=edCamera.z;
    const hitPx=14; // px workspace

    // ¿Hay grupo activo? → comprobar handles
    if(edMultiSel.length){
      const bb=_msBBox();

      // ── Handle de rotación del grupo ───────────────────────────
      // Mismo sistema que individual: offset desde centro bbox en px workspace
      // El handle está en (cx, y0 - 28/z) en coords workspace
      const _rotHx_f = bb.cx;
      const _rotHy_f = bb.cy - (bb.h*ph/2 + 28/z)/ph;
      const _ddx=(c.nx-_rotHx_f)*pw, _ddy=(c.ny-_rotHy_f)*ph;
      if(Math.hypot(_ddx,_ddy)<hitPx){
        // Snapshot de cada miembro
        edMsGesture={
          type:'rotate',
          startAngle: Math.atan2((c.ny-bb.cy)*ph, (c.nx-bb.cx)*pw),
          bbCx:bb.cx, bbCy:bb.cy,
          items: edMultiSel.map(idx=>({
            idx,
            x:edLayers[idx].x, y:edLayers[idx].y,
            rotation:edLayers[idx].rotation||0,
          }))
        };
        edIsDragging=true; // reutilizamos flag para que edOnEnd guarde historial
        return;
      }

      // ── Handles de escala del bbox ──────────────────────────────
      for(const h of _msHandlePositions(bb)){
        const _dx=(c.nx-h.x)*pw, _dy=(c.ny-h.y)*ph;
        if(Math.hypot(_dx,_dy)<hitPx){
          // Esquina opuesta = anchor fijo
          const anchorX = bb.cx*2 - h.x;
          const anchorY = bb.cy*2 - h.y;
          edMsGesture={
            type:'scale',
            corner:h.c,
            anchorX, anchorY,
            startX:c.nx, startY:c.ny,
            bbW:bb.w, bbH:bb.h,
            bbX0:bb.x0, bbY0:bb.y0,
            items: edMultiSel.map(idx=>({
              idx,
              x:edLayers[idx].x, y:edLayers[idx].y,
              width:edLayers[idx].width, height:edLayers[idx].height,
            }))
          };
          edIsDragging=true;
          return;
      ...
      // ── Dentro del bbox o de un miembro → drag ──────────────────
      const inBbox = c.nx>=bb.x0&&c.nx<=bb.x1&&c.ny>=bb.y0&&c.ny<=bb.y1;
      const inMember = edMultiSel.some(idx=>edLayers[idx]&&edLayers[idx].contains(c.nx,c.ny));
      if(inBbox||inMember){
        edMsGesture={
          type:'drag',
          startX:c.nx, startY:c.ny,
          items: edMultiSel.map(idx=>({idx, x:edLayers[idx].x, y:edLayers[idx].y}))
        };
        edIsDragging=true;
        return;
      }
    ...
    // Fuera de cualquier handle/miembro → iniciar rubber band
    _msClear();
  ...
  if(_la && _la.type!=='bubble'){
    ...
    const hitPx = _isT ? 22 : 14;  // px workspace

    // ── Handle de rotación ──────────────────────────────────────────────────
    // El render dibuja el círculo en espacio local del objeto en (0, -h/2 - 28/z).
    // h = la.height*ph (px workspace). El contexto ya tiene setTransform(z,...),
    // por lo que 28/z en ese contexto equivale a 28px físicos = 28/z px workspace.
    // La posición en fracción de página (igual que c.nx/c.ny):
    //   local (0, -(la.height/2*ph + 28/z))  →  rotar por rot  →  sumar centro
    // En fracción: offset_ws = la.height/2*ph + 28/z  (px workspace)
    //              offset_fx = offset_ws / ph           (frac página Y)
    //              offset_fy_x = offset_ws / pw * sin  (frac página X)
    if(!_isT){
      const _rot  = (_la.rotation||0)*Math.PI/180;
      const _offWs = _la.height/2*_ph + 28/edCamera.z;   // px workspace desde centro a handle
      // Rotar vector (0, -_offWs) por _rot → en px workspace
      const _hx_ws =  Math.sin(_rot) * _offWs;
      const _hy_ws = -Math.cos(_rot) * _offWs;
      // Convertir a fracción de página para comparar con c.nx/c.ny
      const _hx_f  = _la.x + _hx_ws / _pw;
      const _hy_f  = _la.y + _hy_ws / _ph;
      // Distancia en px workspace (mismo sistema que hitPx)
      const _ddx = (c.nx - _hx_f) * _pw;
      const _ddy = (c.ny - _hy_f) * _ph;
      if(Math.hypot(_ddx, _ddy) < hitPx){
        edIsRotating = true;
    ...
    // ── Handles de escala (getControlPoints, ignorando 'rotate') ──────────
...
function edOnMove(e){
  ...
  if(edActiveTool==='multiselect'){
    ...
    if(edMsGesture){
      ...
      if(g.type==='drag'){
        const dx=c.nx-g.startX, dy=c.ny-g.startY;
        for(const s of g.items){
          edLayers[s.idx].x=s.x+dx;
          edLayers[s.idx].y=s.y+dy;
        }
      ...
      if(g.type==='rotate'){
        // Ángulo actual del puntero respecto al centro del bbox
        const curAngle=Math.atan2((c.ny-g.bbCy)*ph, (c.nx-g.bbCx)*pw);
        const delta=curAngle-g.startAngle; // radianes
        const cosd=Math.cos(delta), sind=Math.sin(delta);
        for(const s of g.items){
          // Vector (cx→obj) en px, rotado por delta
          const dx_px=(s.x-g.bbCx)*pw, dy_px=(s.y-g.bbCy)*ph;
          edLayers[s.idx].x = g.bbCx + (dx_px*cosd - dy_px*sind)/pw;
          edLayers[s.idx].y = g.bbCy + (dx_px*sind + dy_px*cosd)/ph;
          edLayers[s.idx].rotation = (s.rotation + delta*180/Math.PI) % 360;
        }
        window._edMoved=true;
        edRedraw(); return;
      }
      ...
      if(g.type==='scale'){
        // Escalado proporcional: ratio entre distancia actual y original al anchor
        const dx0=(g.startX-g.anchorX)*pw, dy0=(g.startY-g.anchorY)*ph;
        const dx1=(c.nx  -g.anchorX)*pw, dy1=(c.ny  -g.anchorY)*ph;
        const len0=Math.hypot(dx0,dy0), len1=Math.hypot(dx1,dy1);
        if(len0<1) return;
        const ratio=len1/len0;
        for(const s of g.items){
          // Escalar posición relativa al anchor + escalar tamaño
          const relX=(s.x-g.anchorX)*pw, relY=(s.y-g.anchorY)*ph;
          edLayers[s.idx].x = g.anchorX + relX*ratio/pw;
          edLayers[s.idx].y = g.anchorY + relY*ratio/ph;
          edLayers[s.idx].width  = s.width  * ratio;
          edLayers[s.idx].height = s.height * ratio;
        }
        window._edMoved=true;
        edRedraw(); return;
      }
...
function edOnEnd(e){
  ...
  // ── MULTISELECCIÓN — confirmar rubber band o finalizar gesto ────────────────
  if(edActiveTool==='multiselect'){
    if(edMsRubber && !edMsGesture){
      // Seleccionar objetos cuyo centro cae dentro del rubber band
      const r=edMsRubber;
      ...
      for(let i=0;i<edLayers.length;i++){
        const la=edLayers[i];
        if(la.type==='draw') continue; // la capa de dibujo no es seleccionable
    ...
    edIsDragging=false; window._edMoved=false;
    return;
  }
  // ── FIN MULTISELECCIÓN ──────────────────────────────────────────────────────
...
function EditorView_init(){
  ...
  $('edMultiSelBtn')?.addEventListener('click',()=>{
    if(edActiveTool==='multiselect') edDeactivateMultiselTool();
    else edActivateMultiselTool();
  });
  ...
  window._edKeyFn = function(e){
    ...
    // M: activar/desactivar multiselección
    if(!ctrl && e.key.toLowerCase() === 'm'){
      if(edActiveTool==='multiselect') edDeactivateMultiselTool();
      else edActivateMultiselTool();
      return;
    }
    // Escape: salir de multiselección si está activa
    if(e.key === 'Escape' && edActiveTool==='multiselect'){
      edDeactivateMultiselTool();
      return;
    }
    if(!ctrl) return;
