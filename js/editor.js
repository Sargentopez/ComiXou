...
function EditorView_init(){
  ...
  // edSetOrientation NO hace fit CSS — esta función es la única responsable.
  ...
  // Forzar primer fit inmediato (para el caso en que el CSS ya esté en caché)
  edFitCanvas(); edRedraw();
  // Y además el reintento con hasta 60 frames (~1s) para CSS cargado dinámicamente
  requestAnimationFrame(() => requestAnimationFrame(() => _edInitFit(60)));
  ...
  // Seguro extra a 800ms y 1500ms: si el canvas sigue pequeño, refitear
  [800, 1500].forEach(ms => {
    setTimeout(() => {
      if (edCanvas && parseInt(edCanvas.style.height || '0') < 100) {
        edFitCanvas(); edRedraw();
      }
    }, ms);
  });
