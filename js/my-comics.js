...
async function _mcRunDiag() {
  ...
  if (user && comics.length) {
    ...
    // Hook para capturar errores internos de bibDownload
    const _diagMsgs = [];
    window._bibDiagLog = msg => _diagMsgs.push(msg);

    // Repetir bibDownload para ANI50 con log de errores internos
    if (user) {
      const _ani = comics.find(c => c.title && c.title.toLowerCase().includes('ani50'));
      if (_ani) {
        L('\n── bibDownload ANI50 con diagnóstico interno ──');
        await SupabaseClient.bibDownload(user.id, _ani.supabaseId || _ani.id).catch(e => L('error: ' + e.message));
        _diagMsgs.forEach(m => L('  INTERNAL: ' + m));
        if (!_diagMsgs.length) L('  Sin errores internos capturados');
      } else {
        L('\n(ANI50 no encontrada en comics locales)');
        L('Obras disponibles: ' + comics.map(c => c.title + '(' + (c.supabaseId||c.id).slice(0,8) + ')').join(', '));
      }
    }
    window._bibDiagLog = null;

    // Test directo: fetch raw de biblioteca y _czDecompress manual
    L('\n── Test descompresión directa ──');
    try {
      const user2 = (typeof Auth !== 'undefined') ? Auth.currentUser?.() : null;
      if (user2) {
        const _BASE = 'https://qqgsbyylaugsagbxsetc.supabase.co/rest/v1';
        const _KEY = 'sb_publishable_1bB9Y8TtvFjhP49kwLpZmA_nTVsE2Hd';
        const _sess = JSON.parse(localStorage.getItem('cs_session')||'null');
        const _tok = _sess?.token || _KEY;
        const _r = await fetch(_BASE + '/biblioteca?author_id=eq.' + user2.id + '&layer_type=eq.gif&limit=3', {
          headers: { 'apikey': _KEY, 'Authorization': 'Bearer ' + _tok }
        });
        const _rows = await _r.json();
        L('Filas gif en biblioteca: ' + _rows.length);
        for (const row of _rows.slice(0,3)) {
          const ld = row.layer_data || '';
          L('  id=' + row.id + ' prefix=' + ld.slice(0,10) + ' len=' + ld.length);
          if (ld.startsWith('gz:')) {
            // Intentar descomprimir manualmente
            try {
              const b64 = ld.slice(3);
              L('  b64 len=' + b64.length + ' rem=' + (b64.length%4));
              const bin = atob(b64.slice(0, Math.min(b64.length, 100)));
              L('  atob primeros 100 chars: OK (bin len=' + bin.length + ')');
              // Probar pako directamente
              if (typeof pako !== 'undefined') {
                const slice = b64.slice(0, 200);
                const rem = slice.length % 4;
                const padded = rem ? slice + '===='.slice(rem) : slice;
                const binFull = atob(padded);
                const u8 = new Uint8Array(binFull.length);
                for (let i=0;i<binFull.length;i++) u8[i]=binFull.charCodeAt(i);
                L('  primeros 200 b64 → ' + u8.length + ' bytes');
              }
              // Intentar descomprimir completo
              const _czRes = await (async (s) => {
                const _PFX = 'gz:';
                if (!s.startsWith(_PFX)) return s;
                const _b64 = s.slice(_PFX.length);
                const CHUNK = 32768;
                let bLen=0; const parts=[];
                for (let i=0;i<_b64.length;i+=CHUNK) {
                  const sl=_b64.slice(i,Math.min(i+CHUNK,_b64.length));
                  const rem=sl.length%4;
                  try {
                    const bn=atob(rem?sl+'===='.slice(rem):sl);
                    const p=new Uint8Array(bn.length);
                    for(let j=0;j<bn.length;j++)p[j]=bn.charCodeAt(j);
                    parts.push(p); bLen+=p.length;
                  } catch(e) { return 'atob_error:'+e.message; }
                }
                const bytes=new Uint8Array(bLen);
                let off=0; for(const p of parts){bytes.set(p,off);off+=p.length;}
                if (typeof pako!=='undefined') {
                  try {
                    const res=new TextDecoder().decode(pako.inflate(bytes));
                    return 'pako_ok:len='+res.length+' start='+res.slice(0,30);
                  } catch(e) { return 'pako_error:'+e.message; }
                }
                return 'no_pako';
              })(ld);
              L('  _czDecompress result: ' + _czRes);
            } catch(e) { L('  test error: ' + e.message); }
          } else {
            L('  JSON directo: ' + ld.slice(0,30));
          }
        }
      }
    } catch(e) { L('Test raw error: ' + e.message); }
  }
